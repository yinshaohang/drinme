...
class Listing {
	...
	public function exec($get_total_count=false) {
		...
		if(version_compare(Settings::get('chevereto_version_installed'), '3.7.0', '>=')) {
			...
			$images_likes_join = 'LEFT JOIN '.$tables['likes'].' ON '.$tables['likes'].'.like_content_type = "image" AND '.$tables['likes'].'.like_content_id = '.$tables['images'].'.image_id';
			if(preg_match('/like_user_id/', $this->where)) {
				$joins['images']['likes'] = $images_likes_join;
			} else if($this->requester) {
				$joins['images']['likes'] = $images_likes_join . ' AND '.$tables['likes'].'.like_user_id = '.$this->requester['id'];
			}
